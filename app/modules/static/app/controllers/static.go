...
func (c Static) Serve(prefix, filepath string) revel.Result {
	...
	// Only run this logic if the request is not telling us to not cache
	if c.Request.Header.Get("Cache-Control") != "no-cache" {
		lastModifiedStamp := finfo.ModTime().UTC().Unix()

		if stamp, _ := strconv.ParseInt(c.Request.Header.Get("If-None-Match"), 10, 64); stamp == lastModifiedStamp {
			c.Response.Status = 304
			return nil
		} else {
			c.Response.Out.Header().Set("ETag", strconv.FormatInt(lastModifiedStamp, 10))
		}
	}
